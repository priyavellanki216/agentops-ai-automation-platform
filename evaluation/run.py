...
def metric_summary(results: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        ...
        "task_success_rate": sum(bool(item["task_success"]) for item in successes) / len(successes) if successes else 0.0,
        "failure_rate": len(failures) / len(results),
        "average_latency_ms": average("latency_ms"),
        "p95_latency_ms": sorted(float(item["latency_ms"]) for item in results if item.get("latency_ms") is not None)[max(0, int(len(results) * .95) - 1)] if any(item.get("latency_ms") is not None for item in results) else 0.0,
    }


def compare_versions(baseline: dict[str, float], current: dict[str, float], thresholds: dict[str, float]) -> dict[str, Any]:
    """Return pass/fail without inventing a baseline or current run."""
    if not baseline or not current:
        return {"status": "not_run", "regressions": [], "message": "Both measured baseline and current metrics are required."}
    regressions: list[dict[str, float | str]] = []
    for metric, minimum in (("task_success_rate", thresholds.get("minimum_task_success", 0.0)), ("groundedness", thresholds.get("minimum_groundedness", 0.0))):
    ...
    if current.get("failure_rate", 0.0) > thresholds.get("maximum_failure_rate", 1.0):
        regressions.append({"metric": "failure_rate", "value": current.get("failure_rate", 0.0), "threshold": thresholds.get("maximum_failure_rate", 1.0)})
    baseline_latency = baseline.get("p95_latency_ms", 0.0)
    if baseline_latency and current.get("p95_latency_ms", 0.0) > baseline_latency * (1 + thresholds.get("maximum_latency_regression", 1.0)):
        regressions.append({"metric": "p95_latency_ms", "value": current.get("p95_latency_ms", 0.0), "baseline": baseline_latency})
...
def main() -> int:
    ...
    thresholds = {"minimum_task_success": .85, "minimum_groundedness": .90, "maximum_failure_rate": .05, "maximum_latency_regression": .20}
    ...
    if args.fail_on_regression and (not results or not baseline):
        comparison = {"status": "fail", "regressions": [{"metric": "evaluation_artifacts", "message": "Measured current and baseline artifacts are required for CI."}]}
    ...
    payload = {"status": comparison["status"], "measured": bool(results), "case_count": len(CASES), "metrics": current, "comparison": comparison, "trace_id": trace_payload["trace_id"], "generated_at": datetime.now(timezone.utc).isoformat(), "cases": [asdict(case) for case in CASES]}
    (root / "results").mkdir(exist_ok=True)
    (root / "reports").mkdir(exist_ok=True)
    (root / "results/latest.json").write_text(json.dumps(payload, indent=2) + "\n")
    (root / "reports/report.md").write_text("# AgentOps evaluation report\n\n" + ("Measured metrics are reported below.\n\n" if results else "No provider-backed run has been supplied; metrics are intentionally not fabricated.\n\n") + f"Case inventory: **{len(CASES)}**\n\nTrace ID: **{trace_payload['trace_id']}**\n\nStatus: **{comparison['status'].upper()}**\n\n```json\n{json.dumps(current, indent=2)}\n```\n")
